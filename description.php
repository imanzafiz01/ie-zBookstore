...
<body>
        ...
        <div id="searchbox" class="container-fluid" style="width:112%;margin-left:-6%;margin-right:-6%;">
            <div>
                <form role="search" action="Result.php" method="post">
                    <input type="text" name="keyword" class="form-control" placeholder="Search for a Book or Author" style="width:80%;margin:20px 10% 20px 10%;">
                </form>
            </div>
        </div>
   </div>
    ...
    $PID=$_GET['ID'];
    $query = "SELECT * FROM products WHERE book_isbn='$PID'";
...
echo '
   ...
   ISBN  '.$row["book_isbn"].'   <hr> 
   TITLE         '.$row["book_tittle"].' <hr> 
   AUTHOR        '.$row["book_author"].' <hr>
   AVAILABLE     '.$row["book_stock"].' <hr> 
   PUBLISHER     '.$row["book_publisher"].' <hr>
                        </pre>
    </div>
  </div>
';
